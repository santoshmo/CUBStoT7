...
def guess_bounding_boxes(bounding_box, keypoints, N=100):
    ''' Estimates bounding boxes of bird parts
    ...
    coordinates = [((float(xx)/N)*(X-x)+x, (float(yy)/N)*(Y-y)+y) for xx in range(N) for yy in range(N)]
    domains = {p:[] for p in keypoints}
